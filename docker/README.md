- [Docker](#docker)
  - [Контейнеры](#контейнеры)
    ...
    - [Просмотр всех контейнеров](#просмотр-всех-контейнеров)
    - [Вход в работающий контейнер](#вход-в-работающий-контейнер)
    - [Интерактивное взаимодействие с контейнером](#интерактивное-взаимодействие-с-контейнером)
    - [Удаление контейнера](#удаление-контейнера)
    - [Остановка всех контейнеров](#остановка-всех-контейнеров)
    - [Commit контейнера](#commit-контейнера)
  - [Образы](#образы)
    - [Список образов](#список-образов)
    - [Удалить образ](#удалить-образ)
    - [Узнать размер образов на диске](#узнать-размер-образов-на-диске)
    - [Создание своих образов](#создание-своих-образов)
      - [История изменения образа](#история-изменения-образа)
      - [dockerignore](#dockerignore)
      - [Многоэтапная сборка](#многоэтапная-сборка)
  - [Работа с файлами](#работа-с-файлами)
    - [Создание volume](#создание-volume)
    - [Прокинуть файл или каталог хоста в контейнер](#прокинуть-файл-или-каталог-хоста-в-контейнер)
    - [Концепция Docker-in-Docker](#концепция-docker-in-docker)
    - [Удаление volume](#удаление-volume)
    - [Скопировать файл из контейнера](#скопировать-файл-из-контейнера)
  - [Переменные окружения](#переменные-окружения)
    - [Прокинуть переменные окружения в контейнер](#прокинуть-переменные-окружения-в-контейнер)
  - [Логи](#логи)
  - [Порты](#порты)
  - [Сети](#сети)
    - [none](#none)
...
#### Многоэтапная сборка
Многоэтапная сборка Docker (multi-stage build) позволяет оптимизировать образы, разделяя процесс сборки на несколько этапов и копируя только необходимые артефакты между ними. Это помогает создавать более компактные и эффективные образы, избегая включения лишних файлов и зависимостей.

Основные концепции многоэтапной сборки
1. Разделение на этапы
2. Именование этапов
3. Копирование артефактов между этапами

Пример dockerfile:

```sh
FROM golang:1.19 AS APP   # 1 этап, даем название APP
WORKDIR /app
COPY go.mod go.mod
COPY main.go main.go
RUN go build

FROM ubuntu:22.04         # 2 этап
WORKDIR /app
EXPOSE 5000
COPY --from=APP /app/main ./main  # копируем файл из 1-го этапа сборки 
ENTRYPOINT ["/app/main"]
...
```
    ...
    &jun - так объявляется якорь(&) и его название
    <<: *jun - ссылка на якорь

[Пример файла yaml](./assets/docker-compose.yaml)
...
Запуск только сервиса db (он же postgres) из файла docker-compose.yaml
```sh
